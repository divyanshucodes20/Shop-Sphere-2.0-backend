...
export const updateProduct = TryCatch(async (req, res, next) => {
  ...
  if (stock !== undefined) {
    // Check if stock is updated and notify users if stock goes from 0 to >0
    if (product.stock === 0 && stock > 0) {
      // Find notifications related to this product
      const notifications = await Notification.find({ productId: id });

      // Send notifications to users
      if (notifications.length > 0) {
        const emailPromises = notifications.map(async (notification: any) => {
          await sendNotificationEmail(notification.email, product.name);
        });
...
export const deleteProduct = TryCatch(async (req, res, next) => {
  ...
  const notifications = await Notification.find({ productId: req.params.id });

  if (notifications.length > 0) await Notification.deleteMany({ productId: req.params.id });
